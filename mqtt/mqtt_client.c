...
/*
/insper/guilhermegg/led

sudo systemctl stop mosquitto
sudo killall mosquitto

mosquitto -c mosquitto_bridge.conf -v [nos downloads]
*/
...
#define WIFI_SSID "lcs"
#define WIFI_PASSWORD "luigi2003"
// #define MQTT_SERVER "broker.mqtt.cool"
#define MQTT_SERVER "10.139.162.156"

#define LED_PIN 15
#define LDR_ADC_CHANNEL 0 // Usando ADC0, que é o GP26
...
#define MQTT_TOPIC_LDR "/insper/guilhermegg/light_sensor"
// #define MQTT_TOPIC_LDR "/light_sensor"
#define MQTT_TOPIC_LED "/insper/guilhermegg/led"
// #define MQTT_TOPIC_LED "/led"
#define MQTT_TOPIC_LED_STATE "/insper/guilhermegg/led/state"
...
#define SENSOR_WORKER_TIME_S 5 // Publica a cada 5 segundos
...
static uint16_t read_luminosity() {
    // Seleciona o canal do ADC (já deve estar configurado no main)
    // adc_select_input(LDR_ADC_CHANNEL); // Não é necessário se for o único sensor
    return adc_read();
}
...
static void control_led(MQTT_CLIENT_DATA_T *state, bool on) {
    ...
    if (on)
        //COMO NAO SEI SE EH O EXTERNO OU NAO cyw43_arch_gpio_put(CYW43_WL_GPIO_LED_PIN, 1);
        gpio_put(LED_PIN, 1);
    else
    ...
    mqtt_publish(state->mqtt_client_inst, full_topic(state, MQTT_TOPIC_LED_STATE), message, strlen(message), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
}
...
static void publish_luminosity(MQTT_CLIENT_DATA_T *state) {
    static uint16_t old_luminosity = 0;
    
    const char *luminosity_key = full_topic(state, MQTT_TOPIC_LDR);
    uint16_t luminosity = read_luminosity();

    // Publica apenas se o valor mudar (ou na primeira vez)
    // Pode-se adicionar um 'limite' (threshold) para evitar publicações por flutuações pequenas
    if (luminosity != old_luminosity) {
        old_luminosity = luminosity;
        
        char lum_str[16];
        snprintf(lum_str, sizeof(lum_str), "%u", luminosity); // Envia o valor bruto (0-4095)
        
        INFO_printf("Publicando %s (luminosidade) para %s\n", lum_str, luminosity_key);
        mqtt_publish(state->mqtt_client_inst, luminosity_key, lum_str, strlen(lum_str), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
    }
}
...
static void sub_unsub_topics(MQTT_CLIENT_DATA_T* state, bool sub) {
    ...
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, MQTT_TOPIC_LED), MQTT_SUBSCRIBE_QOS, cb, state, sub);
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags) {
    ...
    if (strcmp(basic_topic, MQTT_TOPIC_LED) == 0)
    {
...
static void luminosity_worker_fn(async_context_t *context, async_at_time_worker_t *worker) {
    ...
    async_context_add_at_time_worker_in_ms(context, worker, SENSOR_WORKER_TIME_S * 1000);
}
...
static void mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status) {
    ...
    // Se houve desconexão
        ...
        INFO_printf("\n--- AVISO: Desconectado do Broker ---\n");
        if (!state->connect_done) {
             INFO_printf("Tentativa de conexao falhou, mas nao e critico ainda.\n");
        }
    }
    // Se houve ERRO (Onde estava o Panic)
    else {
        INFO_printf("\n=============================================\n");
        INFO_printf("ERRO DE CONEXAO MQTT DETECTADO!\n");
        INFO_printf("Codigo de Status: %d\n", status);
        INFO_printf("=============================================\n");
        
        // Diagnostico rapido baseada no codigo
        if (status == 256) INFO_printf("Diagnostico: Protocolo recusado (Versao errada?)\n");
        if (status == 257) INFO_printf("Diagnostico: ID recusado (Tente mudar o nome do device)\n");
        if (status == 258) INFO_printf("Diagnostico: Servidor indisponivel\n");
        if (status == 259) INFO_printf("Diagnostico: Credenciais erradas (Usuario/Senha)\n");
        if (status == 260) INFO_printf("Diagnostico: Nao autorizado (Firewall ou ACL)\n");
        // Timeout geralmente nao vem aqui, mas se vier eh em torno de 257/260 dependendo da lib
    }
}
*/
...
int main(void) {
    ...
    // Tenta conectar num loop até conseguir
    int tentativas = 0;
    while (true) {
        tentativas++;
        // Tenta conectar (30s de timeout)
        int erro = cyw43_arch_wifi_connect_timeout_ms(WIFI_SSID, WIFI_PASSWORD, CYW43_AUTH_WPA2_AES_PSK, 30000);
        
        if (erro == 0) {
            INFO_printf("\nConectado ao Wifi na tentativa %d!\n", tentativas);
            break; // Sai do loop e continua o programa
        } else {
            // Se falhar, imprime o código do erro para sabermos o motivo
            printf("Falha na tentativa %d. Codigo de erro: %d\n", tentativas, erro);
            printf("Tentando novamente em 2 segundos...\n");
            sleep_ms(2000); 
        }
    }
    //*cyw43_arch_enable_sta_mode();
